...
const EditGameModal = ({ 
    ...
    useEffect(() => {
        if (isOpen && game) {
            const gameCategoryIds = game.categorias && game.categorias.length > 0
                ? categories
                    .filter(cat => game.categorias.includes(cat.nombre))
                    .map(cat => cat.id_category)
                : [];

            const gameEditorId = game.editor_nombre
                ? editors.find(ed => ed.nombre === game.editor_nombre)?.id_editor || null
                : null;

            setGameData({
                ...game,
                id_game: game.id_game,
                categorias: gameCategoryIds,
                editor_id: gameEditorId,
                fecha_lanzamiento: game.fecha_lanzamiento ? new Date(game.fecha_lanzamiento).toISOString().split('T')[0] : ''
            });
            setImageUrl(game.gameBanner || '');
            setImagePreview(game.gameBanner || '');
        }
    }, [isOpen, game, categories, editors]);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 relative">
                ...
                <h2 className="text-2xl font-bold mb-6">Editar Juego [{gameData.id_game}]</h2>
